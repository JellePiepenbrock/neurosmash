...
dd = torch.load('training_dones.pt')[:, 1:]
print(dd[0, -1])
...
# print(z2.shape)
z = torch.Tensor(pca.fit_transform(z2_pca).reshape(20, 20, z_size)).cuda()
print(z.shape)
...
class MDNRNN(nn.Module):
    def __init__(self, z_size, n_hidden=256, n_gaussians=5, n_layers=1):
        ...
        self.fc4 = nn.Linear(n_hidden, 1) # predict done state
    ...
    def forward(self, x, h):
        ...
        return (pi, mu, sigma), (h, c), torch.sigmoid(self.fc4(h[0]))
...
for epoch in range(epochs):
    ...
    for i in range(0, z.size(1) - seqlen, seqlen):
        ...
        targets = z[:, (i + 1):(i + 1) + seqlen, :-1]
        done_targets = dd[:, (i + 1):(i + 1) + seqlen].cuda()
        # print(done_targets)
        # print(targets.shape)
        # Forward pass
        hidden = detach(hidden)
        (pi, mu, sigma), hidden, done = model(inputs, hidden)
        # print(targets)

        # print(done)
        loss1 = criterion(targets, pi, mu, sigma)
        loss2 = 100 * criterion2(done, done_targets)
        loss = loss1 + loss2
    ...
    if epoch % 100 == 0:
        print('Epoch [{}/{}], Loss: {:.4f}, Doneloss: {:.4f}'
              .format(epoch, epochs, loss.item(), loss2.item()))
...
(pi, mu, sigma), _, _ = model(x, hidden)
original_x = pca.inverse_transform(x[:, :, :z_size].cpu().detach().numpy()).reshape(3, 64, 64) * 1000
matplotlib.pyplot.imshow(np.transpose(np.array(original_x, "uint8"), (1, 2, 0)))
plt.title("Original")
plt.show()
